...
export function TVChart({ symbol, timeframe, marketType }: { symbol: string; timeframe: string; marketType?: string }) {
  ...
  useEffect(() => {
    ...
    s.innerHTML = JSON.stringify({
      ...
      save_image: false,
      // EMA 9 and EMA 15 only
      studies: [
        { id:'MAExp@tv-basicstudies', inputs:{ length:9  }, override:{ 'Plot.color':'#00ff88','Plot.linewidth':2 } },
        { id:'MAExp@tv-basicstudies', inputs:{ length:15 }, override:{ 'Plot.color':'#ff8800','Plot.linewidth':2 } },
      ],
