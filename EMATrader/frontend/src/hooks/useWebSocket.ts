      ...
      s.onmessage = ev => {
        try {
          ...
          if (m.type === 'EMA_SIGNALS_UPDATE') {
            ...
            // Fire notification for strong signals
            for (const [sym, sig] of Object.entries(m.data) as any) {
              if (sig.direction !== 'NEUTRAL' && sig.strength >= 60 && sig.confirmationOk) {
                notify(`📊 ${sym}: ${sig.direction} signal — Angle ${sig.angle.toFixed(0)}° | Strength ${sig.strength}%`, 'signal')
              }
            }
          }
